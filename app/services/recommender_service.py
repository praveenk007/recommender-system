...
from app.algo import Correlation
...
def create_model_v2(mongo, request_json):
    ...
    Correlation.PearsonCorrelation().correlate(train_data_grouped, reco_meta['user_field'], reco_meta['item_field'], reco_meta['features'][0]['id'], reco_meta['features'][0]['weight'])
...
def recommend_similar(mongo):
    ...
    train_data, test_data = train_test_split(all_data, test_size = 0.10, random_state=0)
    popularity_reco = Recommenders.PopularityRecommend()
